...
public class beeColony {
	...
	double ub = 5.12; /*
					 * upper bound of the parameters. lb and ub can be defined
					 * as arrays for the problems of which parameters have
					 * different bounds
					 */
	...
	double Foods[][] = new double[FoodNumber][D]; /*
												 * Foods is the population of
												 * food sources. Each row of
												 * Foods matrix is a vector
												 * holding D parameters to be
												 * optimized. The number of rows
												 * of Foods matrix equals to the
												 * FoodNumber
												 */
	double f[] = new double[FoodNumber]; /*
										 * f is a vector holding objective
										 * function values associated with food
										 * sources
										 */
	double fitness[] = new double[FoodNumber]; /*
												 * fitness is a vector holding
												 * fitness (quality) values
												 * associated with food sources
												 */
	double trial[] = new double[FoodNumber]; /*
											 * trial is a vector holding trial
											 * numbers through which solutions
											 * can not be improved
											 */
	double prob[] = new double[FoodNumber]; /*
											 * prob is a vector holding
											 * probabilities of food sources
											 * (solutions) to be chosen
											 */
	double solution[] = new double[D]; /*
										 * New solution (neighbour) produced by
										 * v_
										 * {ij}=x_{ij}+\phi_{ij}*(x_{kj}-x_{ij})
										 * j is a randomly chosen parameter and
										 * k is a randomlu chosen solution
										 * different from i
										 */
	...
	int neighbour, param2change; /*
								 * param2change corrresponds to j, neighbour
								 * corresponds to k in equation
	...
	void MemorizeBestSource() {
		...
		for (i = 0; i < FoodNumber; i++) {
			if (f[i] < GlobalMin) {
				...
				drawing.update(problem_to_solve);
				test.addLog("Daha Ýyi Zaman Bulundu: "+(int)GlobalMin);
			}
		}
	}
	...
	void init(int index) {
		...
		f[index] = calculateFunction(solution,index);
	...
	void initial() {
		D = problem_to_solve.getBoyutSayisi();
		int i;
		for (i = 0; i < FoodNumber; i++) {
			init(i);
		}
	...
	void SendEmployedBees() {
		...
		for (i = 0; i < FoodNumber; i++) {
			...
			ObjValSol = calculateFunction(solution,i);
	...
	void SendOnlookerBees() {
		...
		while (t < FoodNumber) {
			...
			r = ((double) Math.random() * 32767 / ((double) (32767) + (double) (1)));
			if (r < prob[i]) /*
							 * choose a food source depending on its probability
							 * to be chosen
							 */
			{
				t++;
				...
				if (FitnessSol > fitness[i]) {
					...
					fitness[i] = FitnessSol;
						 ...
						 * if the solution i can not be improved, increase its
						 * trial counter
						 */
					trial[i] = trial[i] + 1;
				}
	...
	double calculateFunction(double sol[],int particle) {
		Algorithms.apply_SPV("func", sol, problem_to_solve.is_sirasi);
		Algorithms.apply_SB(problem_to_solve, problem_to_solve.is_sirasi, problem_to_solve.makine_sirasi, sol);
		Algorithms.localSearch(problem_to_solve, problem_to_solve.is_sirasi, problem_to_solve.makine_sirasi);
		double result = problem_to_solve.yayilma_zamani;
		return result;
	}
